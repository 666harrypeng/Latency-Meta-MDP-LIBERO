...
def test_l3_admission_preflight_uses_all_train_masters_and_keeps_validation_closed(
    ...
    preflight = admission_runner.build_l3_admission_preflight(
...
def test_l3_admission_checkpoint_restores_without_a_fold_identity(tmp_path: Path) -> None:
    ...
    from latency_meta_mdp.belief.action_conditioned_jepa.admission_runner import (
        ...
        load_l3_admission_training_config,
        write_l3_admission_training_checkpoint,
    )
    ...
    restored = load_l3_admission_training_checkpoint(
        output_dir=output,
        model=restored_model,
...
def test_formal_validation_gate_opens_only_at_the_fixed_epoch_75() -> None:
    ...
    from latency_meta_mdp.belief.action_conditioned_jepa.admission_runner import (
        load_l3_admission_training_config,
        require_l3_formal_validation_gate,
    )
    from latency_meta_mdp.belief.action_conditioned_jepa.training import (
        L3AdmissionProgress,
    )
    ...
    def progress(epoch: int) -> L3AdmissionProgress:
        return L3AdmissionProgress(
            completed_epochs=epoch,
            optimizer_steps=epoch * 176,
            examples_seen=epoch * 176 * 256,
            model_seed=27,
    ...
    with pytest.raises(RuntimeError, match="epoch-75"):
        require_l3_formal_validation_gate(progress(74), config=config)
    require_l3_formal_validation_gate(progress(75), config=config)
...
def test_formal_validation_records_are_not_loaded_before_the_gate(
    ...
    config = admission_runner.load_l3_admission_training_config(
        Path("configs/training/action_conditioned_jepa/l3_admission.yaml")
    )
    calls = []
    ...
    def progress(epoch: int):
        return admission_runner.L3AdmissionProgress(
            completed_epochs=epoch,
            optimizer_steps=epoch * 176,
            examples_seen=epoch * 176 * 256,
            model_seed=7,
    ...
    with pytest.raises(RuntimeError, match="epoch-75"):
        admission_runner.load_l3_formal_validation_records(
            inputs="inputs",
    ...
    assert admission_runner.load_l3_formal_validation_records(
        inputs="inputs",
        episode_ids=("validation-a", "validation-b"),
...
def test_l3_admission_cli_executes_the_final_runner(
    ...
    monkeypatch.setattr(cli, "execute_l3_admission_job", fake_execute, raising=False)
    assert cli.main(
        [
            "--project-root",
            ".",
