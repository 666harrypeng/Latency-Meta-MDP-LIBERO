...
def test_formal_collection_config_names_only_the_paired_group_unit() -> None:
    ...
    config = load_formal_corpus_config(
        ROOT / "configs/data/source_corpus/panda_ball_formal_source_pilot.yaml"
    )
...
def test_fresh_unsplit_smoke_config_is_one_complete_paired_block() -> None:
    ...
    config = load_formal_corpus_config(
        ROOT / "configs/data/source_corpus/panda_ball_formal_source_smoke.yaml"
    )
...
def test_formal_unsplit_source_config_targets_100_by_4_with_bounded_reserves() -> None:
    ...
    config = load_formal_corpus_config(
        ROOT / "configs/data/source_corpus/panda_ball_formal_source_100x4.yaml"
    )
