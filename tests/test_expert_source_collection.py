...
def _publication_fixture(*, boundary_count: int = 2):
    ...
    split = MasterTaskSplitPlan(
        schema_version=1,
        split_id="source-pilot-split",
        corpus_id=formal.corpus_id,
        train_master_task_indices=(0,),
        validation_master_task_indices=(1,),
    )
    ...
    metadata = replace(
        ...
        master_task_split_plan_sha256=split.sha256,
    )
    episode = replace(
        source,
        metadata=metadata,
    ...
    return request, split, source_config, task, episode
...
def test_source_publication_contains_only_success_data_and_central_metadata(
    ...
    assert provenance["split_plan"] == split.to_mapping()
    assert pq.read_table(target / "meta/episodes.parquet")["episode_id"].to_pylist() == [
        episode.metadata.episode_id
    ]


def test_source_publication_rejects_incomplete_blocks_and_split_drift(tmp_path: Path) -> None:
    """Break caught: a partial task block or wrong split is finalized as complete."""
    from latency_meta_mdp.expert_realization.source_corpus.collection import publish_source_corpus
...
def test_source_publication_requires_episode_and_task_table_identity_match(
    ...
    wrong_task = replace(task, split="validation")
    with pytest.raises(ValueError, match="split plan"):
        publish_source_corpus(
            target=tmp_path / "wrong-split",
            request=request,
            source_config=config,
            split_plan=split,
            task_entries=(wrong_task,),
            admitted_episodes=(_admitted(episode),),
            collection_summary=_summary(),
        )
