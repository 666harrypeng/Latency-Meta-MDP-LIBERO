...
def test_swept_clearance_detects_between_tick_collision() -> None:
    ...
    report = evaluate_moving_target_swept_clearance(
        reference_positions_world=np.array([[-0.10, 0.0, 0.0], [0.10, 0.0, 0.0]], dtype=np.float64),
...
def test_contact_admission_is_phase_and_link_aware() -> None:
    ...
    from latency_meta_mdp.data.collection.safety import ContactKind, contact_is_allowed

    assert contact_is_allowed(ContactKind.PAD_BALL, StructuredExpertPhase.CLOSE_STABILIZE) is True
