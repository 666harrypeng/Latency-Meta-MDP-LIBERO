...
from latency_meta_mdp.cli.probe_flow_belief_law_retention import build_parser
...
def test_law_retention_cli_uses_locked_defaults() -> None:
    ...
    assert args.multilaw_config == Path(
        "configs/belief/dinov3_flow_belief_multilaw_v3.yaml"
    )
    assert args.probe_config == Path(
        "configs/analysis/dinov3_flow_belief_law_retention_probe_v1.yaml"
    )
