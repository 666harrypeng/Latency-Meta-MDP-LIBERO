...
def profile():
    return {
        "schema": 1, "unit": "conditioned_vla_rtc_equivalent",
        "reference_seconds": 0.2, "binding": {"protocol_id": "rtc-test"},
        "weights": {"vla_conditioned": 1., "vla_clean": 0.8, "forecast": 0.2,
                    "decode": 0.03, "meta_q": 0.01, "rule": 0.001,
                    "bootstrap_clean": 0.5},
    }
...
def episode(*, combined=False, q=True):
    ...
    events.append(event("meta_decision", 10, q_values=[0., 1.] if q else None, launch=False))
    if not combined:
        events.append(event("forecast_prepare", 14, available=True))
    events += [event("meta_decision", 14, q_values=[0., 1.] if q else None, launch=True),
               event("forecast" if combined else "forecast_decode", 14,
                     available=True, request_id=0),
    ...
    return {
        ...
        "stage_events": events, "bootstrap_calls": 1, "policy_calls": 1,
        "forecast_calls": 1 if combined else 2, "forecast_decodes": 1,
        "pending_at_terminal": True, "terminated": True, "truncated": False,
        "decision_transitions": [{"start_tick": 10, "end_tick": 14, "action": "wait"},
...
def test_cost_rejects_ambiguous_or_incompatible_records(fault):
    ...
    if fault == "duplicate":
        ...
        d["stage_events"].append({"stage": "forecast", "formal_tick": 14,
                                  "request_id": 0, "available": True})
    with pytest.raises(ValueError):
        episode_cost_components(d, profile())


def test_profile_validation_and_budget_update(tmp_path):
    from latency_meta_mdp.meta_cost import budget_multiplier_step, load_cost_profile
    ...
    with pytest.raises(ValueError):
        load_cost_profile(p)
    assert budget_multiplier_step(0.1, 12., 10., 0.1) == pytest.approx(0.12)
    assert budget_multiplier_step(0.01, 0., 10., 0.1) == 0
    with pytest.raises(ValueError):
        budget_multiplier_step(0.1, 1., 0., 0.1)
