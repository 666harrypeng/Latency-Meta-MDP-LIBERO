...
def _parquet_bytes(
    episode: FormalSourceSynchronizedEpisode, config: SourceCorpusConfig
) -> bytes:
    output = io.BytesIO()
    pq.write_table(
    ...
    record = _json_bytes(
        {
            ...
            "physical_events": [
                event.to_mapping() for event in loaded.episode.physical_events
            ],
...
def load_source_success_payload(
    ...
    *,
    ...
    payloads = {
        name: _read_regular_file_nofollow(root / name, name=name) for name in _FILES
    }
    manifest = _strict(
    ...
    if record["schema_version"] != 1 or record["source_config_sha256"] != (
    ...
    events = tuple(
        StructuredPhysicalEventRecord.from_mapping(value)
        for value in record["physical_events"]
    )
