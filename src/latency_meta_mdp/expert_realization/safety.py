...
class CompiledSafetyGeometry:
    ...
    def __post_init__(self) -> None:
        ...
        if self.environment_geom_ids & (
            self.robot_assembly_geom_ids | self.ball_geom_ids
        ):
...
def classify_contact_pair(
    ...
    if geom1 != geom2 and geom1 in geometry.robot_assembly_geom_ids and (
        geom2 in geometry.robot_assembly_geom_ids
    ):
...
def phase_order_is_valid(phase_sequence: tuple[str, ...]) -> bool:
    ...
    try:
        ...
        return False
    return values[0] == 0 and values[-1] == len(canonical) - 1 and all(
...
def build_actual_rollout_safety_report(
    *,
    ...
    outside_contact = tuple(
        ...
        if item.phase
        in {StructuredExpertPhase.SMOOTH_APPROACH, StructuredExpertPhase.GRASP_FUNNEL}
    )
    ...
    pre_handoff = tuple(
        item
        for item in decisions
        if handoff_tick is None or item.source_formal_tick < handoff_tick
    )
    first_funnel = next(
    ...
    return ActualRolloutSafetyReport(
        ...
        maximum_reference_tracking_error_m=maximum_error(
            outside_contact, "position_error_base"
        ),
