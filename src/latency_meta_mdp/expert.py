    ...
    def next_action(
        ...
        *,
        ...
        if handoff_state is HandoffState.DRIVEN:
            current_pregrasp = ball_position + np.array(
                [0.0, 0.0, self.config.pregrasp_height_m]
            )
            if (
            ...
            if (
                ...
                and np.linalg.norm(eef_position - ball_position)
                < self.config.grasp_tolerance_m
            ):
