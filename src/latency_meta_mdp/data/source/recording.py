...
from latency_meta_mdp.data.collection.executor import StructuredExpertDecision
...
from latency_meta_mdp.envs.outcomes import OutcomeEvent
...
class SourceEpisodeRecorder:
    ...
    def append_boundary(
        ...
        *,
        ...
        qualification = StructuredQualificationRecord(
            ...
            commanded_motion_segment_index=int(_commanded_scalar(snapshot, "segment_index")),
    ...
    def append_shared_transition(self, snapshot: BoundarySnapshot, action: np.ndarray) -> None:
        if not isinstance(snapshot, BoundarySnapshot):
            raise TypeError("snapshot must be a BoundarySnapshot")
...
def execute_structured_source_recording(
    *,
    ...
    from latency_meta_mdp.data.collection.robot_bridge import PandaPlanningBridge
    from latency_meta_mdp.data.collection.rollout import _execute_structured_realization
    from latency_meta_mdp.data.collection.safety import ActualRolloutSafetyReport
