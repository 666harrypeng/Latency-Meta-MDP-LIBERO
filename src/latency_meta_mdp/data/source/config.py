...
class SourceCorpusConfig:
    ...
    @property
    def sha256(self) -> str:
        payload = json.dumps(self.to_mapping(), sort_keys=True, separators=(",", ":")).encode(
            "utf-8"
        )
...
def load_master_task_split_plan(path: Path) -> MasterTaskSplitPlan:
    ...
    return MasterTaskSplitPlan.from_mapping(_load_yaml(path, expected, name="master task split"))
...
def load_source_execution_config(path: Path) -> SourceExecutionConfig:
    ...
    return SourceExecutionConfig.from_mapping(_load_yaml(path, expected, name="source execution"))
