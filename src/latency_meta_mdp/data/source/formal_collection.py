...
class CompletedMasterBlock:
    ...
    def __post_init__(self) -> None:
        ...
        by_level = {
            level: sorted(row.accepted_slot for row in self.successes if row.level == level)
            for level in (1, 2, 3)
        }
...
class FirstQualifiedPlan:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.reference, SelectedReference) or (
            self.reference.expert_realization_key != self.candidate.expert_realization_key
        ):
            raise ValueError("selected reference does not match its candidate")
        if type(
            self.attempted_candidate_indices
        ) is not tuple or self.attempted_candidate_indices != tuple(
            range(self.candidate.candidate_index + 1)
        ):
