...
from latency_meta_mdp.data.source.formal_planning_runtime import (
...
def _success_payload_path(
    ...
    relative = f"payloads/successful/L{level}/task-{logical:06d}/draw-{draw_index:04d}"
...
def _reference_from_status(status: Any, *, workspace: Any, structured: Any) -> Any:
    from latency_meta_mdp.data.collection.planner import load_single_candidate_result
    from latency_meta_mdp.data.collection.selector import freeze_selected_reference
    ...
    for level in formal_request.config.levels:
        ...
        while len(level_successes) < formal_request.config.realizations_per_task:
            ...
            admitted_reference_paths = tuple(
                _reference_from_status(
                    row, workspace=workspace, structured=structured
                ).eef_positions_world
