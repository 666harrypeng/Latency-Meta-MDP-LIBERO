...
from latency_meta_mdp.data.collection.artifacts import _check_final_root, _hash_file
...
class SourceCorpusManifest:
    ...
    def from_mapping(cls, mapping: Any) -> SourceCorpusManifest:
        ...
        if raw["schema_version"] != schema_version or raw["complete"] is not True:
            raise ValueError("source manifest is not a complete supported corpus")
...
class VerifiedSourceCorpus:
    ...
    def episode_ids(self, *, level: int, split: str | None = None) -> tuple[str, ...]:
        ...
        return tuple(
            sorted(
                ...
                if row["level"] == level and (split is None or row["split"] == split)
            )
        )
    ...
    def read_fields(
        ...
        *,
        ...
        if (
            type(allowed_roles) is not frozenset
            or not allowed_roles
            or any(not isinstance(role, SourceFieldRole) for role in allowed_roles)
        ):
...
def load_verified_source_corpus(
    ...
    if manifest.legacy_split:
        ...
        payload = json.dumps(legacy_config, sort_keys=True, separators=(",", ":")).encode()
        if (
    ...
    tasks, episodes, events = _load_metadata_tables(root, schema_version=manifest.schema_version)
