...
def build_expert_episode_runtime(
    *,
    ...
    paths = {
        ...
        "motion": root / f"configs/tasks/moving_ball/motion/dynamic_grasp_lift_l{spec.level}.yaml",
    ...
    metadata = EpisodeMetadata(
        ...
        instruction=(
            task_spec.instruction if spec.level == 0 else "Grasp the moving ball and lift it."
        ),
        level=spec.level,
...
def collect_expert_episode(
    *,
    ...
    expert_config = load_expert_config(
        project_root.resolve() / "configs/data/expert/panda_ball_feedback_v1.yaml"
    )
