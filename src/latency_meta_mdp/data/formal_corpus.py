...
def materialize_formal_corpus(
    *,
    ...
    expected = {
        (level, seed) for level in levels for seed in range(seed_start, seed_start + seed_count)
    }
        ...
        sources.append(
            {
                ...
                "implementation_source_sha256": source.get("implementation_source_sha256"),
            }
        )
    ...
    try:
        for (level, seed), row in sorted(episodes.items()):
            ...
            for name in names:
                ...
                artifacts[destination.relative_to(building).as_posix()] = sha256_file(destination)
