...
from latency_meta_mdp.data.collection.trajectory_intent import PlannedMotionIntent
...
def osc_action_from_reference(
    *,
    ...
    achieved_position = _vector(achieved_position_world, name="achieved_position_world")
    target_position = _vector(target_position_world, name="target_position_world")
    achieved_orientation = _rotation(achieved_orientation_world, name="achieved_orientation_world")
    target_orientation = _rotation(target_orientation_world, name="target_orientation_world")
    world_to_base = _rotation(world_to_base_rotation, name="world_to_base_rotation")
    ...
    normalized_unclipped = (
        desired_output - output_bias
    ) / output_weight * input_weight + input_bias
    normalized = np.clip(
        normalized_unclipped,
...
class StructuredExpertExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        expected_duration = (intent.approach.funnel_entry_target_tick - decision_source_tick) * 0.02
        if (
    ...
    def _funnel_target(
        ...
        descent = (
            10.0 * descent_progress**3 - 15.0 * descent_progress**4 + 6.0 * descent_progress**5
        )
        capture_height = self.intent.grasp_funnel.eef_capture_offset_world[2]
        height = capture_height + (self.intent.strategy.funnel_entry_height_m - capture_height) * (
            1.0 - descent
        )
        horizon = bounded_prediction_horizon(
            source_tick=snapshot.formal_tick_index,
    ...
    def _entry_is_achieved(
        ...
        current_entry = np.asarray(snapshot.object_body_pos, dtype=np.float64) + np.array(
            [0.0, 0.0, self.intent.strategy.funnel_entry_height_m],
            dtype=np.float64,
        )
    ...
    def next_action(
        ...
        *,
        ...
        if source_tick < entry_target_tick:
            ...
            effective_close_deadline = funnel.handoff_deadline_tick - funnel.close_dwell_ticks - 2
            ...
            if handoff_state is HandoffState.PHYSICAL and close_dwell_complete:
                ...
                target = self._lift_start_position_world + funnel.lift_relative_displacement_world
            else:
                phase = StructuredExpertPhase.CLOSE_STABILIZE
                horizon = bounded_prediction_horizon(
                    source_tick=source_tick,
