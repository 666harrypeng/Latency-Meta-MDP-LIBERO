...
def planner_candidate_seeds(key: ExpertRealizationKey, *, candidate_count: int) -> tuple[int, ...]:
...
def validate_scheduled_joint_path(
    ...
    *,
    ...
    if any(
        value.dtype != np.float64 or value.shape != (7,) or not np.all(np.isfinite(value))
...
class PlannerCandidate:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            type(value) is not float or not np.isfinite(value) or value < 0 for value in metrics
        ):
...
def load_planner_candidates(
    ...
    for index, row in enumerate(rows):
        ...
        key = ExpertRealizationKey.from_mapping(row.pop("expert_realization_key"))
...
def load_single_candidate_result(
    ...
    with np.load(io.BytesIO(array_payload), allow_pickle=False) as source:
        arrays = {name: np.array(source[name], copy=True) for name in source.files}
    key = ExpertRealizationKey.from_mapping(metadata.pop("expert_realization_key"))
    candidate = PlannerCandidate(
        expert_realization_key=key,
...
def run_curobo_candidate_process(
    ...
    *,
    ...
    try:
        ...
        key = ExpertRealizationKey.from_mapping(request["expert_realization_key"])
