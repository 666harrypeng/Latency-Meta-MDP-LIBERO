...
def _profile(task_instance: MaterializedTaskInstance):
    config = load_motion_config(
        ...
        / "configs/tasks/moving_ball/motion"
        / f"dynamic_grasp_lift_l{task_instance.task_instance_id.level}.yaml"
    )
...
def _validate_workspace(
    ...
    task = load_task_spec(
        task_instance.project_root / "configs/tasks/moving_ball/task/dynamic_grasp_lift_l0.yaml"
    )
...
def build_trajectory_intent(
    ...
    latest_safe_entry_tick = (
        handoff_deadline_tick - strategy.funnel_descent_ticks - strategy.close_dwell_ticks - 1
    )
