...
def _verify_request_sources(root: Path, request: TimingCalibrationAttemptRequest) -> None:
    expected = {
        ...
        / f"configs/tasks/moving_ball/motion/dynamic_grasp_lift_l{request.level}.yaml",
    ...
    mismatches = [
        name for name, path in expected.items() if _sha256(path) != getattr(request, name)
    ]
    if mismatches:
...
def run_matched_feedback_calibration_attempt(
    *,
    ...
    expert = ScriptedBallExpert(
        ...
        config=load_expert_config(root / "configs/data/expert/panda_ball_feedback_v1.yaml"),
    )
