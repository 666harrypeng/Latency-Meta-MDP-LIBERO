...
from latency_meta_mdp.envs.outcomes import EpisodeOutcomeTracker, OutcomeCriteria
from latency_meta_mdp.envs.snapshots import BoundarySnapshot, BoundarySnapshotter
from latency_meta_mdp.envs.task import TaskSpec, load_task_spec, make_dynamic_grasp_lift_environment
...
def _load_configuration(project_root: Path, level: int) -> _LoadedConfiguration:
    ...
    paths = {
        ...
        "motion": root / f"configs/tasks/moving_ball/motion/dynamic_grasp_lift_l{level}.yaml",
        "runtime": root / "configs/runtime/robosuite_v1.yaml",
        "controller": root / "configs/runtime/control/panda_osc_pose_delta_v1.yaml",
        "structured": root / "configs/data/expert_realization/panda_ball_structured.yaml",
        "pilot": root / "configs/data/collection/panda_ball_structured_pilot.yaml",
    }
    ...
    *,
    ...
    try:
        ...
        if safety_bridge is not None:
            from latency_meta_mdp.data.collection.actual_physics import (
