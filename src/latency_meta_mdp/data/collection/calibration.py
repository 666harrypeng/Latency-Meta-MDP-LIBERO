...
_CALIBRATION_SOURCE_PATHS = (
    "pyproject.toml",
    "uv.lock",
    "src/latency_meta_mdp/io/artifacts.py",
    "src/latency_meta_mdp/envs/backend.py",
    "src/latency_meta_mdp/envs/control.py",
    "src/latency_meta_mdp/envs/expert.py",
    "src/latency_meta_mdp/runtime/handoff.py",
    "src/latency_meta_mdp/envs/motion.py",
    "src/latency_meta_mdp/envs/outcomes.py",
    "src/latency_meta_mdp/envs/snapshots.py",
    "src/latency_meta_mdp/envs/task.py",
    "src/latency_meta_mdp/runtime/timing.py",
    "src/latency_meta_mdp/runtime/diagnostics/calibrate_structured_expert_timing.py",
    "src/latency_meta_mdp/data/collection/artifacts.py",
    "src/latency_meta_mdp/data/collection/calibration.py",
    "src/latency_meta_mdp/data/collection/calibration_worker.py",
    "src/latency_meta_mdp/data/collection/config.py",
    "src/latency_meta_mdp/data/collection/contracts.py",
    "src/latency_meta_mdp/data/collection/recording_contracts.py",
    "src/latency_meta_mdp/data/collection/shared_prefix.py",
...
class TimingCalibrationRow:
    ...
    def __post_init__(self) -> None:
        ...
        if any(type(value) is not int or value < 0 or value % 2_000 for value in present_times):
...
class TimingCalibrationReport:
    ...
    def __post_init__(self) -> None:
        ...
        expected_per_level = {
            level: sum(row.level == level and row.terminal_status == "success" for row in self.rows)
            for level in self.config.levels
        }
...
def build_timing_calibration_report(
    *,
    ...
    if not isinstance(config, TimingCalibrationConfig):
        raise TypeError("config must be TimingCalibrationConfig")
    expected = tuple(
        (task_index, level) for task_index in config.logical_task_indices for level in config.levels
    )
    ...
    *,
    ...
    completed = subprocess.run(
        [
            str(worker_python),
            "-m",
            "latency_meta_mdp.data.collection.calibration_worker",
            "--request",
            str(request_path),
            "--result",
            str(result_path),
...
def publish_timing_calibration_report(
    ...
    from latency_meta_mdp.data.collection.artifacts import _publish_tree
...
def load_verified_timing_calibration_report(root: Path) -> TimingCalibrationReport:
    ...
    if (
        ...
        or ImplementationIdentity.from_mapping(manifest["implementation"]) != report.implementation
    ):
...
def collect_timing_calibration(
    *,
    ...
    common_hashes = {
        name: hashlib.sha256(path.read_bytes()).hexdigest() for name, path in common_paths.items()
    }
    requests = tuple(
