...
def _episode_arrays(episode: SynchronizedEpisode) -> dict[str, np.ndarray]:
    ...
    arrays: dict[str, np.ndarray] = {
        ...
        "boundary_time_us": np.asarray([record.time_us for record in boundaries], dtype=np.int64),
        ...
        "gripper_qpos": np.stack([record.deployment.gripper_qpos for record in boundaries]),
        "gripper_qvel": np.stack([record.deployment.gripper_qvel for record in boundaries]),
        ...
        "expert_phase": np.asarray([record.expert_audit.phase.value for record in transitions]),
        ...
        arrays.update(
            {
                ...
                "object_velocity": np.stack([record.object_velocity for record in records]),
                ...
                "handoff_state": np.asarray([record.handoff_state.value for record in records]),
                "relative_geometry": np.stack([record.relative_geometry for record in records]),
            }
        )
    ...
    if episode.metadata.record_profile.includes_control_debug:
        ...
        valid = np.asarray([record.applied_reference is not None for record in records], dtype=bool)
        references = np.full((len(records), episode.metadata.action_dim), np.nan, dtype=float)
        ...
        arrays.update(
            {
                "actuator_ctrl": np.stack([record.actuator_ctrl for record in records]),
