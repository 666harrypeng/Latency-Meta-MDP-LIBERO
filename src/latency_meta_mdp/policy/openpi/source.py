...
from latency_meta_mdp.policy.openpi.patches import apply_openpi_patch
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="metamdp-openpi-copy-") as directory:
        ...
        try:
            ...
            sys.path[:] = [
                entry for entry in sys.path if not Path(entry).resolve().is_relative_to(copied)
            ]
            if "beartype" in sys.modules:
                # The pinned checker caches hints by representation across source copies.
                # Invalidate those hints, rather than disabling checks on reloaded classes.
...
def _purge_worktree_modules(worktree: Path, modules_before: set[str]) -> None:
    ...
    for name, module in tuple(sys.modules.items()):
        ...
        if belongs_to_worktree or name.startswith(
            (
                "latency_meta_mdp.openpi_",
                "latency_meta_mdp.policy.openpi.",
                "latency_meta_mdp.legacy.policy.openpi_",
            )
        ):
