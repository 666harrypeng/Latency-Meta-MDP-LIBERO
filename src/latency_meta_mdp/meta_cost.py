...
COMPONENTS = (
    "vla_conditioned", "vla_clean", "forecast", "decode", "meta_q", "rule", "bootstrap_clean",
)
...
def validate_cost_profile(profile):
    ...
    if (set(weights) != set(COMPONENTS) or weights["vla_conditioned"] != 1
...
def budget_multiplier_step(value, mean_cost, budget, step_size):
    if (not all(math.isfinite(v) for v in (value, mean_cost, budget, step_size))
            or min(value, mean_cost, step_size) < 0 or budget <= 0):
...
def episode_cost_components(result, profile):
    ...
    bindings = {**profile["binding"], **profile.get("policy_bindings", {}).get(
        "conditioned" if conditioned else "clean", {})}
    ...
    if rows is not None:
        ...
        if (any(r["end_tick"] <= r["start_tick"] for r in rows)
                or any(a["end_tick"] != b["start_tick"] for a, b in zip(rows, rows[1:]))):
    ...
    if (len({e["formal_tick"] for e in prepared}) != len(prepared)
            or not (set(combined) | set(decodes)) <= set(launches)):
    ...
    if (len(bootstrap) != result["bootstrap_calls"]
            or len(bootstrap) != sum(e["stage"] == "bootstrap_return" for e in events)):
    ...
    return {
        "episode_cost": total, "initial_cost": initial, "transition_costs": costs,
        "counts": counts,
        "auxiliary_wall_ns": {stage: sum(e.get("wall_duration_ns", 0) for e in events
                                         if e["stage"] == stage)
                              for stage in ("forecast_history", "observation",
                                            "control_and_next_observation")},
    }
