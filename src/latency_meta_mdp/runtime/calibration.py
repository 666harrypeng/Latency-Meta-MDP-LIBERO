...
def _g1_source_sha256() -> str:
    paths = [
        ...
        _PROJECT_ROOT / "src/latency_meta_mdp/io/artifacts.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/runtime/calibration.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/runtime/diagnostics/calibrate_timing.py",
    ]
...
def _run_lane(*, seed: int, camera_width: int, camera_height: int) -> _LaneResult:
    ...
    try:
        ...
        command_error = max(
            float(
                np.max(np.abs(snapshot.object_qpos - snapshot.commanded_world["cube_target_qpos"]))
            )
            for snapshot in snapshots
        )
        body_command_error = max(
            float(
                np.max(
                    np.abs(
                        snapshot.object_body_pos - snapshot.commanded_world["cube_target_qpos"][:3]
                    )
                )
            )
            for snapshot in snapshots
        ...
        sim_time_error = max(
            abs(snapshot.sim_time_seconds - snapshot.time_us / 1_000_000) for snapshot in snapshots
        )
        state_arrays = [
            array
            for snapshot in snapshots
