...
from latency_meta_mdp.envs.backend import AppliedControlSample, PreparedPhysicsPoint
from latency_meta_mdp.envs.control import ActionContract
from latency_meta_mdp.envs.motion import DrivenBallWorld, motion_sample_to_mapping
from latency_meta_mdp.envs.outcomes import EpisodeOutcomeTracker, OutcomeStatus
    ...
    def on_physics_point(self, point: PreparedPhysicsPoint) -> dict[str, np.ndarray]:
        ...
        if point.at_formal_boundary:
            committed = self.outcome_tracker.evaluate_boundary(
                ...
                request_handoff=(
                    self.state is HandoffState.DRIVEN and self.outcome_tracker.handoff_eligible
                ),
            )
            if committed:
