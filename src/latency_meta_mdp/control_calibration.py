...
def _implementation_source_sha256() -> str:
    paths = (
        ...
        _PROJECT_ROOT / "src/latency_meta_mdp/control.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/control_calibration.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/snapshots.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/task.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/timing.py",
        _PROJECT_ROOT / "src/latency_meta_mdp/cli/calibrate_control.py",
    )
...
def run_control_calibration(
    *,
    ...
    try:
        ...
        boundary_trace.append(
            {
                ...
                "gripper_width_m": float(
                    abs(initial_gripper_qpos[0] - initial_gripper_qpos[1])
                ),
                "phase": "initial",
        ...
        for phase in _phase_plan(contract, calibration):
            ...
            for _ in range(phase.ticks):
                ...
                orientation_errors.append(
                    _rotation_distance(desired_orientation, current.eef_xmat)
                )
        ...
        arm_saturated_physics_step_fraction = float(
            np.mean(np.any(saturation_matrix, axis=1))
        )
        ...
        return_error = float(
            np.linalg.norm(
                np.asarray(return_reference["end_eef_position_m"]) - initial.eef_pos
            )
        )
        ...
        block(
            float(rotation["rotation_response_rad"])
            < calibration.min_rotation_response_rad,
            "rotation_response",
        )
        block(
        ...
        block(
            float(closed["end_gripper_width_m"])
            > calibration.max_closed_gripper_width_m,
            "gripper_close",
        )
        block(
            float(reopened["end_gripper_width_m"])
            < calibration.min_reopened_gripper_width_m,
            "gripper_reopen",
        )
