"""Probe whether a frozen Flow Belief Encoder retains its latency-law input."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--multilaw-config",
        type=Path,
        default=Path("configs/legacy/belief/dinov3_flow_belief_multilaw_v3.yaml"),
    )
    parser.add_argument(
        "--split-config",
        type=Path,
    ...
    parser.add_argument(
        "--evaluation-config",
        type=Path,
        default=Path("configs/legacy/analysis/dinov3_flow_belief_multilaw_evaluation_v1.yaml"),
    )
    parser.add_argument(
        "--probe-config",
        type=Path,
        default=Path("configs/legacy/analysis/dinov3_flow_belief_law_retention_probe_v1.yaml"),
    )
...
def main() -> None:
    from latency_meta_mdp.legacy.belief.flow.law_retention_run import (
        run_law_retention_probe,
    )

    args = build_parser().parse_args()
    manifest = run_law_retention_probe(
