...
def load_belief_training_split_plan(path: Path) -> BeliefTrainingSplitPlan:
    ...
    if not isinstance(raw, dict) or set(raw) != set(BeliefTrainingSplitPlan.__dataclass_fields__):
...
def write_belief_training_index_artifact(
    *,
    ...
    action_path = project / "configs/runtime/control/panda_osc_pose_delta_v1.yaml"
    ...
    try:
        ...
        with (staging / "index.npz").open("xb") as handle:
            ...
            os.fsync(handle.fileno())
        artifact_hashes = {
            name: sha256_file(staging / name) for name in ("index.npz", "summary.json")
        }
        _write_json(
