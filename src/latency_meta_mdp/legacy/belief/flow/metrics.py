...
def energy_score(
    *,
    ...
    for start in range(0, len(draws), 4):
        ...
        context_scores.append(np.sum(probability[start:stop] * per_query, axis=-1))
...
def sample_distribution_metrics(
    *,
    ...
    coverage_68 = (
        np.sum(expanded_weight * ((expected >= lower_68) & (expected <= upper_68))) / denominator
    )
    coverage_95 = (
        np.sum(expanded_weight * ((expected >= lower_95) & (expected <= upper_95))) / denominator
    )
    dispersion = np.std(draws, axis=2)
    dispersion_mean = float(np.sum(expanded_weight * dispersion) / denominator)
    collapsed_fraction = float(np.sum(expanded_weight * (dispersion < 1e-5)) / denominator)
    return {
