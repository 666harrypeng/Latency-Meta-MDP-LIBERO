...
from latency_meta_mdp.legacy.belief.flow.multilaw_evaluation_config import (
...
def run_law_retention_probe(
    *,
    ...
    gate_checks = {
        "effective_mean_mae": (
            primary_metrics["effective_mean_mae_ms"] <= probe_config.effective_mean_mae_ms_max
        ),
    ...
    try:
        ...
        artifacts = {
            path.name: sha256_file(path) for path in sorted(building.iterdir()) if path.is_file()
        }
        _write_json(
