...
from latency_meta_mdp.legacy.belief.flow.context_sampling import load_verified_flow_quality_level
...
def _masked_latency_weighted_context_mean(
    ...
    for branch in range(array.shape[1]):
        ...
        if np.any(contexts):
            result[branch] = np.mean(numerator[contexts, branch] / denominator[contexts, branch])
    return result
    ...
    *,
    ...
    for branch_index, branch_id in enumerate(branch_ids):
        ...
        result["branches"][branch_id] = {
            ...
            "qpos_conditioning_gain_latency_weighted_rad": float(weighted_qpos_gain[branch_index]),
            "qpos_effect_cosine": (
    ...
    for phase in phases:
        ...
        result["phases"][phase] = {
            branch_id: {
                ...
                "qpos_error_latency_weighted_rad": float(phase_qpos_error[branch_index]),
                "object_position_error_m": float(
                ...
                "qpos_conditioning_gain_latency_weighted_rad": float(phase_qpos_gain[branch_index]),
            }
            for branch_index, branch_id in enumerate(branch_ids)
        }
    return result
...
def evaluate_buffer_counterfactual_level(
    *,
    ...
    for context_index, context_identity in enumerate(contexts):
        ...
        actions = (
            (branch_actions[context_index] - normalization.action_mean) / normalization.action_std
        ).astype(np.float32)
        latency = np.repeat(
        ...
        physical = normalized.cpu().numpy() * normalization.target_std + normalization.target_mean
    ...
    try:
        ...
        eligible = (
            not provenance.dirty and sim_manifest.get("eligible") is True and checkpoint_eligible
        )
        blockers = []
        if provenance.dirty:
