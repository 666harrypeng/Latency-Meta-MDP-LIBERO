...
from latency_meta_mdp.data.expert_collection import ExpertEpisodeSpec, build_expert_episode_runtime
...
def phase_candidates_from_episode_arrays(
    *,
    ...
    for phase in _PHASES:
        ...
        ticks = ticks[
            (ticks >= minimum_source_tick) & (ticks + required_real_action_count <= len(phases))
        ]
        if len(ticks) == 0:
            continue
    ...
    *,
    ...
    try:
        ...
        with (building / "counterfactuals.npz").open("xb") as handle:
            ...
            os.fsync(handle.fileno())
        artifacts = {
            name: sha256_file(building / name) for name in ("contexts.json", "counterfactuals.npz")
        }
        manifest = {
            **manifest_fields,
            "schema_version": 1,
            ...
            "maximum_expert_reference_max_abs": float(np.max(simulation.expert_reference_max_abs)),
            "artifacts": artifacts,
        }
