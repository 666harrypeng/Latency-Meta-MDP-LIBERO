...
from latency_meta_mdp.data.expert_collection import ExpertEpisodeSpec
...
_CONFIG_RELATIVE_PATHS = {
    ...
    "task": "configs/tasks/moving_ball/task/dynamic_grasp_lift_l0.yaml",
    "control": "configs/runtime/control/panda_osc_pose_delta_v1.yaml",
    "expert": "configs/data/expert/panda_ball_feedback_v1.yaml",
}
_REQUIRED_ARRAYS = {
...
def _config_paths(project_root: Path, *, level: int) -> dict[str, Path]:
    result = {name: project_root / relative for name, relative in _CONFIG_RELATIVE_PATHS.items()}
    result["motion"] = (
        project_root / f"configs/tasks/moving_ball/motion/dynamic_grasp_lift_l{level}.yaml"
    )
    return result
...
def _verify_nested_episode(
    *,
    ...
    if not episode_manifest_path.is_file() or sha256_file(episode_manifest_path) != artifacts.get(
        relative_manifest
    ):
    ...
    for episode in episodes:
        ...
        for phase in config.phases:
            ...
            interval_candidates = phase_candidates[
                (phase_candidates >= interval.minimum) & (phase_candidates <= interval.maximum)
            ]
