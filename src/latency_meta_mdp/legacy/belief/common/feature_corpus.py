...
from latency_meta_mdp.runtime.latency_law import TruncatedBetaLatencyLaw, load_latency_law
...
def load_level_feature_belief_corpus(
    *,
    ...
    action_contract = load_action_contract(
        project_root.resolve() / "configs/runtime/control/panda_osc_pose_delta_v1.yaml"
    )
    records = tuple(
        _build_record(
