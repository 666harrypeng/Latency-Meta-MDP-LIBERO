...
from latency_meta_mdp.legacy.return_belief_geometry import RETURN_STATE_DIM
...
class BeliefEncoder(nn.Module):
    ...
    def __init__(self, config: GaussianBeliefConfig) -> None:
        ...
        self.patch_pool_queries = nn.Parameter(torch.empty(2, config.patch_pool_query_count, dim))
        ...
        self.belief_queries = nn.Parameter(torch.empty(config.belief_token_count, dim))
...
def diagonal_gaussian_nll(
    *,
    ...
    return 0.5 * torch.mean(torch.square(standardized) + 2.0 * log_std + math.log(2.0 * math.pi))
