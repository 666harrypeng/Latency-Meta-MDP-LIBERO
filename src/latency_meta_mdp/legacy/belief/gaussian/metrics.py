...
def gaussian_prediction_metrics(
    *,
    ...
    for name, columns, unit, multiplier in _GROUPS:
        ...
        standardized_mean = float(np.sum(expanded_weight * standardized) / denominator)
        ...
        skew = float(np.sum(expanded_weight * np.power(centered / scale, 3)) / denominator)
...
def weighted_normalized_gaussian_nll(
    *,
    ...
    element = 0.5 * (np.square(residual) + 2.0 * logarithm + np.log(2.0 * np.pi))
