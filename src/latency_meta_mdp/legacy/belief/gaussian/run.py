"""Atomic per-level runner for Gaussian predictive return-belief training."""
...
from latency_meta_mdp.legacy.belief.gaussian.training import train_level_gaussian_belief
...
def train_gaussian_belief_run(
    *,
    ...
    if (
        ...
        raise ValueError("Gaussian belief levels must be sorted unique values from 1, 2, 3")
    inputs = {
        ...
        "belief_config": belief_config_path.resolve(),
    }
    for path in inputs.values():
        if not path.is_file():
            raise FileNotFoundError(f"Gaussian belief input does not exist: {path}")
    spec = load_vision_encoder_spec(inputs["vision_config"])
    config = load_gaussian_belief_config(inputs["belief_config"])
    provenance = collect_implementation_provenance(project_root)
    target = output_dir.resolve()
    if target.exists():
        raise FileExistsError(f"Gaussian belief run already exists: {target}")
    ...
    try:
        for level in selected_levels:
            ...
            level_manifest = train_level_gaussian_belief(
                corpus=corpus,
                config=config,
        ...
        manifest = {
            "schema_version": 1,
            "format_id": "gaussian_belief_run_v1",
