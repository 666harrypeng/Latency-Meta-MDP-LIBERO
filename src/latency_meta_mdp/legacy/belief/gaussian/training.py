...
from latency_meta_mdp.legacy.belief.gaussian.metrics import (
...
from latency_meta_mdp.legacy.belief_training_data import InteractionMode
...
def _to_device(batch: GaussianBeliefBatch, device: str) -> dict[str, torch.Tensor]:
    return {
        ...
        "latency_probabilities": batch.latency_probabilities.to(device=device, non_blocking=True),
        ...
        "query_probabilities": batch.query_probabilities.to(device=device, non_blocking=True),
    }
...
def _weighted_nll_torch(
    *,
    ...
    element = 0.5 * (torch.square(standardized) + 2.0 * log_std + math.log(2.0 * math.pi))
...
def _evaluate(
    ...
    *,
    ...
    subsets = {
        ...
        "post_handoff": (~absorbing_mask & (interaction_mode == InteractionMode.GRASPED)),
