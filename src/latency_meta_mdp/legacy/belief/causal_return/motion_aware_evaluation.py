...
def match_baseline_contexts(
    *,
    ...
    if len(candidate_source_sha256) != 64 or candidate_source_sha256 != baseline_source_sha256:
...
def evaluate_motion_aware_admission(
    *,
    ...
    if level == 3:
        ...
        after_velocity = candidate_metrics["transition"]["after_2_to_5"]["object_velocity_rmse_m_s"]
        far_velocity = candidate_metrics["transition"]["far"]["object_velocity_rmse_m_s"]
    ...
    *,
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            position = (
                estimate.object_position_normalized.cpu().numpy() * normalization.position_std
                + normalization.position_mean
            )
            velocity = (
                estimate.object_velocity_normalized.cpu().numpy() * normalization.velocity_std
    ...
    admission = evaluate_motion_aware_admission(
        ...
        require_formal_episode_count=not bool(checkpoint_manifest.get("bounded_review", False)),
    )
