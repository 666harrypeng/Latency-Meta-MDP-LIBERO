...
from latency_meta_mdp.legacy.belief.causal_return.contracts import InformationStateConfig
    ...
    *,
    ...
    with torch.inference_mode():
        for batch in loader:
            ...
            object_mean = torch.from_numpy(np.array(normalization.object_mean, copy=True)).to(
                device
            )
            means.append((estimate.object_state_mean * object_std + object_mean).cpu().numpy())
    ...
    try:
        ...
        artifacts = {
            name: sha256_file(building / name) for name in ("predictions.npz", "metrics.json")
        }
        _write_json(
