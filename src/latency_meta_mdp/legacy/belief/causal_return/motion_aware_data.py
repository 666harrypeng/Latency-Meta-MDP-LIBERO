...
def _physical_state_values(
    *,
    ...
    return (
        ...
        np.asarray(episode.supervision.object_pose[source_tick, :3], dtype=np.float32),
        np.asarray(episode.supervision.object_velocity[source_tick, :3], dtype=np.float32),
    )
...
def build_motion_aware_history_corpus(
    *,
    ...
    for source_record in source.records:
        ...
        records.append(MotionAwareHistoryEpisodeRecord(source=source_record, indices=indices))
