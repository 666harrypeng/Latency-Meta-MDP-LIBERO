...
from latency_meta_mdp.legacy.belief_data import BeliefEpisodeView
...
def build_return_state_stream(episode: BeliefEpisodeView) -> np.ndarray:
    ...
    gripper_width_velocity = deployment.gripper_qvel[:, 0] - deployment.gripper_qvel[:, 1]
    ...
    if state.shape != (episode.boundary_count, RETURN_STATE_DIM) or not np.all(np.isfinite(state)):
...
def build_return_contexts(
    *,
    ...
    if round(latency_law.control_tick_seconds * 1_000_000) != (temporal_contract.formal_tick_us):
    ...
    for source_tick in range(interval.minimum, interval.maximum + 1):
            ...
            action_targets = np.stack(
                [
                    episode.expert_actions[
                        target_tick : target_tick + temporal_contract.prediction_horizon
                    ]
        ...
        contexts.append(
            ReturnContext(
                ...
                return_physical_handoff=(episode.supervision.handoff_state[targets] == "physical"),
...
def build_absorbing_return_contexts(
    *,
    ...
    relative_positions = tail_view.extend_boundary_array(episode.supervision.relative_geometry)
...
def fit_state_normalization(
    ...
    *,
    ...
    if states.ndim != 2 or states.shape[1] != RETURN_STATE_DIM or not np.all(np.isfinite(states)):
    ...
    *,
    ...
    if total_variance <= 1e-15:
        ...
        if standard_deviation <= 1e-15:
            ...
            excess_kurtosis = float(np.sum(probabilities * standardized**4) - 3.0)
            order = np.argsort(standardized)
            ordered_values = standardized[order]
            ordered_probabilities = probabilities[order]
            midpoint_cdf = np.cumsum(ordered_probabilities) - 0.5 * ordered_probabilities
            ...
            gaussian_quantile_rmse = float(
                np.sqrt(np.mean((empirical_quantiles - ndtri(quantile_probabilities)) ** 2))
            )
    ...
    if len(central) < 2:
        ...
        tortuosity = 1.0 if path_length <= 1e-15 else float(path_length / max(chord_length, 1e-12))
    return StateGeometryMetrics(
        ...
        gripper_width_velocity_spread=_weighted_rms_spread(states[:, 15:16], probabilities),
        object_position_rms_spread=_weighted_rms_spread(states[:, 16:19], probabilities),
        object_velocity_rms_spread=_weighted_rms_spread(states[:, 19:22], probabilities),
        ...
        physical_handoff_probability=float(probabilities[context.return_physical_handoff].sum()),
        absorbing_target_probability=float(probabilities[context.return_absorbing].sum()),
    )
...
def measure_action_compatibility(
    ...
    *,
    ...
    return ActionCompatibilityMetrics(
        translation_rms_deviation=float(
            np.sqrt(np.sum(probabilities[:, None, None] * centered[:, :, :3] ** 2) / 50)
        ),
        rotation_rms_deviation=float(
            np.sqrt(np.sum(probabilities[:, None, None] * centered[:, :, 3:6] ** 2) / 50)
        ),
        gripper_disagreement_probability=_gripper_disagreement(actions[:, :, 6], probabilities),
