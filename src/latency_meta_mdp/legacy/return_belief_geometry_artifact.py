...
def _rows_to_arrays(
    *,
    ...
    return {
        f"{prefix}_episode_id": np.asarray([row["episode_id"] for row in rows], dtype=np.str_),
        f"{prefix}_level": np.asarray([row["level"] for row in rows], dtype=np.int8),
        f"{prefix}_source_tick": np.asarray([row["source_tick"] for row in rows], dtype=np.int64),
        f"{prefix}_source_phase": np.asarray([row["source_phase"] for row in rows], dtype=np.str_),
        **{
            f"{prefix}_metric_{name}": np.asarray([row[name] for row in rows], dtype=np.float64)
            for name in metric_names
        },
    }
...
def _build_summary(
    *,
    ...
    if absorbing_tail is not None:
        ...
        summary["action_contract_config_sha256"] = sha256_file(action_contract_path)
...
def write_return_belief_geometry_artifact(
    *,
    ...
    if config.tail_protocol_id is not None:
        action_contract_path = project / "configs/runtime/control/panda_osc_pose_delta_v1.yaml"
    ...
    normalization = fit_state_normalization(tuple(state_streams), floor=config.state_scale_floor)
    state_rows: list[dict[str, Any]] = []
    action_rows: list[dict[str, Any]] = []
    ...
    arrays = {
        **_rows_to_arrays(prefix="state", rows=state_rows, metric_names=_STATE_METRICS),
        **_rows_to_arrays(prefix="action", rows=action_rows, metric_names=_ACTION_METRICS),
    }
