...
from latency_meta_mdp.meta.replay import FEATURE_FORMAT
...
PHYSICAL_KEYS = (
    ...
    "maximum_steps",
    "profile_sha256",
    "bootstrap_verification_sha256",
    "protocol_id",
    "conditioning",
)


def create_success_replay(entries, output_path):
    """Index initial collection without copying shards or recomputing visual features."""
    output_path = Path(output_path)
    if output_path.exists():
        raise FileExistsError(output_path)
    masters = {"train": set(), "validation": set()}
    paths, binding = set(), None
    normalized = []
    for entry in entries:
        entry = {key: str(Path(entry[key]).resolve()) for key in ("result", "replay")}
        if entry["replay"] in paths:
            raise ValueError("duplicate replay source")
        paths.add(entry["replay"])
        meta, _ = audited_episode(entry)
        identity = {key: meta["identity"][key] for key in PHYSICAL_KEYS}
        if binding is not None and identity != binding:
            raise ValueError("initial replay mixes frozen environments")
        binding = identity
        masters[meta["case"]["meta_partition"]].add(meta["case"]["master_index"])
        normalized.append(entry)
    if not all(masters.values()) or masters["train"] & masters["validation"]:
        raise ValueError("initial replay needs disjoint train and validation masters")
    manifest = {
        "schema": 2,
        "status": "completed",
        "revision": 0,
        "episodes": normalized,
        "expected_episodes": len(normalized),
        **{part + "_masters": sorted(values) for part, values in masters.items()},
    }
...
def append_success_replay(parent_path, entries, output_path):
    ...
    for entry in entries:
        ...
        meta, _ = audited_episode(entry)
        if (
            meta["case"]["meta_partition"] != "train"
...
def audited_episode(entry):
    ...
    if (
        ...
        or np.any(idx >= size)
        or np.any(nxt >= size)
        or np.any((nxt < 0) != term)
        or not np.isin(fields["action"], [0, 1]).all()
    ):
    ...
    if (
        not np.array_equal(nxt[:-1], idx[1:])
...
def load_success_replay(manifest_path, *, visual_cache=None, cost_profile=None):
    ...
    for episode_id, entry in enumerate(entries):
        ...
        if visual_cache is None:
            ...
            from latency_meta_mdp.meta.visual_replay import cache_visual_shard
        ...
        if cost_profile is not None:
            ...
            if cost_report["transition_costs"] is None or len(
                cost_report["transition_costs"]
            ) != len(nxt):
    ...
    if (
        not all(masters.values())
        or masters["train"] & masters["validation"]
        or any(sorted(masters[k]) != sorted(manifest[k + "_masters"]) for k in masters)
    ):
    ...
    if visual_cache is not None:
        from latency_meta_mdp.meta.visual_replay import VisualReplay
    ...
    return (
        ...
        {k: np.concatenate([r[k] for r in rows]) for k in rows[0]},
        binding,
        inventory,
