...
class MetaEpisodeReplay:
    ...
    def save(self, path: Path, *, metadata):
        ...
        arrays["metadata_utf8"] = np.frombuffer(
            json.dumps(
                {"feature_format": FEATURE_FORMAT, "replay_schema": 2, **metadata}, allow_nan=False
            ).encode(),
            dtype=np.uint8,
        )
...
class ProbabilisticLaunchScheduler:
    ...
    def __call__(self, state, observation, belief):
        greedy = self.scheduler(state, observation, belief) if self.scheduler is not None else False
