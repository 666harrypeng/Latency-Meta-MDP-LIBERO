...
from latency_meta_mdp.meta.learning_state import (
    learning_rate_at,
    load_learning_state,
    save_learning_state,
)
from latency_meta_mdp.meta.q import MetaQNetwork, fitted_q_targets
...
def meta_batch_predictions(model, target, visual, vector, records, admissible, indices, cfg):
    ...
    if cfg.get("target_method") == "greedy_trace":
        from latency_meta_mdp.meta.returns import greedy_trace_predictions
...
def main():
    ...
    parser.add_argument("--config", "--training-config", dest="training_config", type=Path)
    ...
    if args.training_config is not None:
        ...
        objective = (
            "rtc_budget_success_v1" if cost_profile is not None else "finite_horizon_success_v2"
        )
        if (
            requested["objective"] != objective
            or requested["gamma"] != 1
            or requested["call_cost"] != 0
            or requested["forecast_cost"] != 0
    ...
    if cost_profile is not None:
        if (
            cfg.get("target_method") != "greedy_trace"
            or type(cfg.get("max_trace_steps")) is not int
            or cfg["max_trace_steps"] < 1
            or not 0 <= cfg.get("trace_decay", -1) <= 1
        ):
            raise ValueError("budget objective requires a valid greedy trace configuration")
        cfg.update(
            cost_profile=cost_profile, budget=args.budget, cost_multiplier=args.cost_multiplier
        )
    ...
    if args.init_from is not None:
        ...
        if (
            source_config["feature_format"] != FEATURE_FORMAT
    ...
    if args.resume_from is not None:
        ...
        if start_step > cfg["updates"]:
            raise ValueError("requested end step precedes restored progress")
    ...
    if args.replay_manifest is not None:
        ...
        if args.resume_from is not None:
            ...
            probes = sorted(
                p
                for p in args.output_dir.glob("probe-*.npz")
        ...
        json.dumps(
            {
                ...
                "replay_storage": (
                    str(visual.device) if isinstance(visual, torch.Tensor) else "mapped_shards"
                ),
            }
        ),
        flush=True,
    )
    last = {"step": start_step, "segment_start_step": start_step, "segment_sample_draws": 0}
    if start_step == cfg["updates"]:
            ...
            if step == start_step + 1 or step % cfg.get("log_interval", 50) == 0:
                ...
                if probe is not None and step % 500 == 0:
                    ...
                    last.update(
                        ...
                        probe_q_min=float(q.min()),
                        probe_q_max=float(q.max()),
                    )
                    if previous_q is not None:
                        last["probe_q_mean_drift"] = float((q - previous_q).abs().mean())
                        last["probe_legal_action_flip_fraction"] = (
                            float((q.argmax(1) != previous_q.argmax(1))[both_legal].float().mean())
                            if both_legal.any()
                            else 0.0
                        )
