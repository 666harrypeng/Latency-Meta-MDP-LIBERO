...
from latency_meta_mdp.expert import ExpertObservation, is_qualified_expert_episode
from latency_meta_mdp.expert_collection import (
    ExpertEpisodeSpec,
    build_expert_episode_runtime,
)
from latency_meta_mdp.latency_client import OneStepLatencyClient
from latency_meta_mdp.latency_harness import (
    FixedDelaySampler,
    HarnessEvent,
    ...
    def validate_exact(self) -> None:
        for name in ParityLaneTrace.array_field_names():
            ...
            if not np.array_equal(direct, harness):
                ...
                raise ValueError(
                    f"zero-delay parity mismatch: {name} at {coordinate} {index}"
                )
        for name in (
            "physical_events",
            "terminal_status",
