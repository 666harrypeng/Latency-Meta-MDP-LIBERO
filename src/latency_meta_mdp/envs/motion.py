...
class MotionConfig:
    ...
    def from_mapping(cls, raw: dict[str, Any]) -> MotionConfig:
        ...
        expected_profile_id = (
            "dynamic_grasp_lift_l0" if level == 0 else f"dynamic_grasp_lift_l{level}_v3"
        )
        ...
        config = cls(
            ...
            max_continuous_acceleration_mps2=scalars.get("max_continuous_acceleration_mps2"),
            l2_waypoint_deviation_range_m=(
                _tuple(
            ...
            segment_count_three_probability=scalars.get("segment_count_three_probability"),
    ...
    def validate(self) -> None:
        ...
        endpoint_x_min, endpoint_x_max, endpoint_y_min, endpoint_y_max = self.endpoint_bounds_xy
        if not (
        ...
        if (
            self.three_segment_first_change_time_us_range[1]
...
class PiecewisePolynomialProfile:
    ...
    def from_mapping(cls, raw: dict[str, Any]) -> PiecewisePolynomialProfile:
        if (
            set(raw) != {"schema_version", "segments", "type", "workspace_z"}
...
def _validate_profile_contract(profile: MotionProfile, config: MotionConfig) -> None:
    ...
    if isinstance(profile, StationaryProfile):
        ...
        if (
            not config.l2_curvature_change_time_us_range[0]
            <= change_time_us
        ...
        for left, right in zip(profile.segments, profile.segments[1:]):
            ...
            cosine = np.clip(
                ...
                / (np.linalg.norm(left.end_velocity_xy) * np.linalg.norm(right.start_velocity_xy)),
                -1.0,
                1.0,
            )
            angle = float(np.degrees(np.arccos(cosine)))
            if (
                not config.turn_angle_degrees_range[0]
                <= angle
    ...
    if config.level:
        ...
        if config.level in {2, 3}:
            ...
            maximum_deviation = (
                ...
                if deviation_range is not None
                else None
            )
...
def _cubic_from_waypoints(
    *,
    ...
    vandermonde = np.stack([np.ones(4), parameters, parameters**2, parameters**3], axis=1)
