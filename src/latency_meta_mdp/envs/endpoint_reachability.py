    ...
    def __post_init__(self) -> None:
        ...
        for name in ("first_contact_us", "stable_grasp_us", "handoff_us"):
            ...
            if value is not None and (
                isinstance(value, bool) or not isinstance(value, int) or value < 0 or value % 2_000
            ):
...
class SafeRectangle:
    ...
    def cell_count(self) -> int:
        return (self.x_index_max - self.x_index_min + 1) * (self.y_index_max - self.y_index_min + 1)
...
def run_endpoint_attempt(
    *,
    ...
    from latency_meta_mdp.envs.config import load_runtime_config
    ...
    from latency_meta_mdp.envs.motion import DrivenBallWorld, StationaryProfile
    ...
    contract = load_action_contract(root / "configs/runtime/control/panda_osc_pose_delta_v1.yaml")
    expert_config = load_expert_config(root / "configs/data/expert/panda_ball_feedback_v1.yaml")
    task_spec = load_task_spec(root / "configs/tasks/moving_ball/task/dynamic_grasp_lift_l0.yaml")
    if (
...
def render_endpoint_reachability_svg(report: Mapping[str, Any]) -> str:
    ...
    for y_index, y_value in enumerate(y_values):
        ...
        lines.append(
            f'<text class="axis" x="{left - 10}" y="{y:.1f}" text-anchor="end">{y_value:.2f}</text>'
        )
    lines.extend(
        [
    ...
    *,
    ...
    report["config_sha256"] = {
        "calibration": sha256_file(
            root / "configs/runtime/control/panda_endpoint_reachability_v1.yaml"
        ),
        "control": sha256_file(root / "configs/runtime/control/panda_osc_pose_delta_v1.yaml"),
        "expert": sha256_file(root / "configs/data/expert/panda_ball_feedback_v1.yaml"),
        "runtime": sha256_file(root / "configs/runtime/robosuite_v1.yaml"),
        "task": sha256_file(root / "configs/tasks/moving_ball/task/dynamic_grasp_lift_l0.yaml"),
    }
    return report
