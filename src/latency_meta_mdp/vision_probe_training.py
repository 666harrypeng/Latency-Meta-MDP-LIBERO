...
from latency_meta_mdp.vision_probe_config import VisionProbeConfig
from latency_meta_mdp.vision_probe_corpus import VisionProbeCorpus
from latency_meta_mdp.vision_probe_data import ProbeSplit
from latency_meta_mdp.vision_probe_metrics import (
...
class _ProbeDataset(Dataset):
    def __init__(
        ...
        *,
        ...
        self.proprio_streams = tuple(
            _proprio_stream(record.episode) for record in corpus.records
        )
        self.target_streams = tuple(
            _target_stream(record.episode) for record in corpus.records
        )
...
def _improvement(probe: dict[str, Any], baseline: dict[str, Any]) -> dict[str, float]:
    ...
    for name in ("object_position", "object_linear_velocity", "relative_position"):
        ...
        result[name] = (
            float("nan")
            if denominator <= 0.0
            else 1.0 - probe[name]["rmse_si"] / denominator
        )
    return result
