...
class MotionAwareTrainingProvenance:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or any(
                not _is_digest(value, length=64)
        ...
        if callable(transition_metadata):
            ...
            metadata = tuple(
                ...
                for sample in (
                    ...
                    for offset in range(len(self.references))
                )
            )
    ...
    def __getitem__(self, offset: int) -> MotionAwareItem:
        ...
        return MotionAwareItem(
            ...
            object_position_target=(
                (position - self.normalization.position_mean)
                / self.normalization.position_std
            ).astype(np.float32),
            object_velocity_target=(
                (velocity - self.normalization.velocity_mean)
                / self.normalization.velocity_std
            ).astype(np.float32),
...
def _batch_values(batch: MotionAwareBatch, *, device: str) -> dict[str, torch.Tensor]:
    return {
        ...
        "object_position_target": batch.object_position_target.to(
            device=device, non_blocking=True
        ),
        "object_velocity_target": batch.object_velocity_target.to(
            device=device, non_blocking=True
        ),
        "exact_transition_mask": batch.exact_transition_mask.to(
            device=device, non_blocking=True
        ),
    }
    ...
    *,
    ...
    for epoch in range(config.max_epochs):
        ...
        train_loss = train_position_sum / train_position_count + (
            train_velocity_sum / train_velocity_count
            if train_velocity_count > 0
            else 0.0
        )
