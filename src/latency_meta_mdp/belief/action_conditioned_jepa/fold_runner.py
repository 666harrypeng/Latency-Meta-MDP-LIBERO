...
def build_temporal_fold_preflight(
    *,
    ...
    training = load_temporal_jepa_training_config(
        root / "configs/training/action_conditioned_jepa/temporal_selection_l3.yaml"
    )
    ...
    development_contexts = sum(
        index.episode_id in development_ids for index in selection.indices
    )
    fold_step_counts = []
    ...
    wandb_config = load_jepa_wandb_config(
        root / "configs/training/action_conditioned_jepa/wandb.yaml"
    )
...
def execute_temporal_fold_job(
    *,
    ...
    deployed_development_corpus = TemporalJepaDeployedEvaluationCorpus(
        development_corpus
    )
    ...
    for epoch in range(progress.completed_epochs, target_epochs):
        ...
        if wandb_run is not None:
            ...
            if "development_monitor" in epoch_payload:
                ...
                wandb_payload["development_d20/source_mean_deployed_latent_rmse"] = (
                    deployed["source_mean_deployed_latent_rmse"]
                )
                wandb_payload["development_d20/source_mean_deployed_proprio_rmse"] = (
                    deployed["source_mean_deployed_proprio_rmse"]
                )
                wandb_payload[
                    "development_d20/source_mean_temporal_quantization_latent_rmse"
                ] = deployed["source_mean_temporal_quantization_latent_rmse"]
                wandb_payload[
                    "development_d20/source_mean_temporal_quantization_proprio_rmse"
