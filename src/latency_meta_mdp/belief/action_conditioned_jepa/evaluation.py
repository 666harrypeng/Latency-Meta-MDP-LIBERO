...
def evaluate_temporal_batch(
    *,
    ...
    persistence_latent_mse = (
        current_visual - batch.target_visual_latents.float()
    ).square().mean(dim=(2, 3, 4))
    persistence_proprio_mse = (
        current_proprio - batch.target_proprio_physical.float()
    ).square().mean(dim=2)
    skill = torch.where(
...
def evaluate_deployed_temporal_batch(
    *,
    ...
    if (
        batch.dense_target_visual_latents.shape[:2]
    ...
    if (
        bool((native_positions >= batch.dense_delay_ticks.numel()).any())
        or not torch.equal(
...
def summarize_deployed_temporal_evaluation(
    ...
    return {
        ...
        "source_mean_assigned_model_proprio_rmse": float(
            model_proprio.mean(axis=1).mean()
        ),
        ...
        "per_delay_temporal_quantization_latent_rmse": quantization_latent.mean(
            axis=0
        ).tolist(),
        "per_delay_temporal_quantization_proprio_rmse": quantization_proprio.mean(
            axis=0
        ).tolist(),
