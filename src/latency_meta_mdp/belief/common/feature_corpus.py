...
class FeatureBeliefCorpus:
    ...
    def materialize(self, split: ProbeSplit, offset: int) -> FeatureBeliefSample:
        ...
        remaining_stop = (
            index.source_tick + self.temporal_contract.remaining_buffer_coverage
        )
        return FeatureBeliefSample(
            ...
            remaining_actions=record.tail.expert_actions[
                index.source_tick:remaining_stop
            ],
            latency_probabilities=self.latency_law.probabilities,
...
def _build_record(
    *,
    ...
    return FeatureBeliefEpisodeRecord(
        ...
        target_state_stream=build_absorbing_return_state_stream(tail).astype(
            np.float32
        ),
        left_contact=tail.extend_boundary_array(
            episode.supervision.left_pad_contact
        ),
        right_contact=tail.extend_boundary_array(
