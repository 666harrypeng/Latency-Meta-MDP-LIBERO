    ...
    *,
    ...
    for path_value in evaluation_manifests:
        ...
        if (
            ...
            != "level_flow_belief_buffer_causality_evaluation_v1"
    ...
    for level in (1, 2, 3):
        ...
        for phase, phase_rows in sorted(summary["phases"].items()):
            for branch_id, branch in sorted(phase_rows.items()):
                ...
                if branch_id != "expert":
                    weighted_gain = float(
                        branch["qpos_conditioning_gain_latency_weighted_rad"]
                    )
    ...
    report = {
        ...
        "maximum_invariant_object_false_coupling_latency_weighted_m": max(
            false_coupling_weighted
        ),
