...
def sequence_indices(next_transition, start_indices, max_steps):
    ...
    if (((next_transition != -1) & (next_transition != positions + 1)).any()
            or (next_transition >= len(next_transition)).any()
            or (start_indices < 0).any() or (start_indices >= len(next_transition)).any()):
...
def trace_targets(reward, discount, next_value, continue_trace, valid_mask):
    if (reward.ndim != 2 or reward.shape[1] < 1 or
            any(x.shape != reward.shape for x in (discount, next_value, continue_trace, valid_mask))
    ...
    if (not all(torch.isfinite(x).all() for x in (r, d, v, c))
            or (d < 0).any() or (d > 1).any() or (c < 0).any() or (c > 1).any()):
        raise ValueError("invalid trace return values")
    result = torch.zeros_like(r[:, 0])
    for t in range(r.shape[1] - 1, -1, -1):
        continuation = (c[:, t] * valid_mask[:, t + 1]
                        if t + 1 < r.shape[1] else torch.zeros_like(result))
        result = r[:, t] + d[:, t] * ((1 - continuation) * v[:, t] + continuation * result)
    return result.clamp(max=1.)
