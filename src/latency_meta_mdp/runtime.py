...
def validate_runtime_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        not isinstance(report["python_version"], str)
        or report["python_version"].split(".")[:2] != ["3", "10"]
    ):
    ...
    if (
        not isinstance(report["implementation_revision"], str)
        or not _REVISION_RE.fullmatch(report["implementation_revision"])
    ):
